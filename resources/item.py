from flask_restful import Resource,reqparse
from flask_jwt import jwt_required
from models.item import ItemModel

class Item(Resource):    
    parser = reqparse.RequestParser()
    parser.add_argument('price',required=True,type=float,help="This field cannot be left blank")
    parser.add_argument('store_id',required=True,type=int,help="Every item needs a store id")

    @jwt_required()
    def get(self,name):        
        name = ItemModel.find_by_name(name)
        if name:
            return(name.json(),200)           
        return({"message":"Item not found"},404)    

    def post(self,name):
        if ItemModel.find_by_name(name):
            return({"message":"Item with name {} already exists.".format(name)},400)        
        data = Item.parser.parse_args()
        item = ItemModel(name,data["price"],data["store_id"])        
        try:         
            item.save_to_db()       
        except:
            return({"message":"An error occured inserting the item"},500)
        return(item.json(),201)
    
    def put(self,name):        
        data = Item.parser.parse_args()        
        item = ItemModel.find_by_name(name)
        
        if item is None:            
            item = ItemModel(name,**data)  # data["price"],data["store_id"] is replaced by **data
        else:
            item.price = data["price"]  
            item.store_id = data["store_id"]      
        item.save_to_db()
        
        return(item.json(),200)

    def delete(self,name):
        item = ItemModel.find_by_name(name)
        if item:
            item.delete_from_db()
        return({"message":"Item Deleted"})        

class ItemList(Resource):
    def get(self):
        return({"items":list(map(lambda x:x.json(),ItemModel.get_items_from_db()))},200)
        #return({"items":items},200)